/**
 * Section components.
 *
 * Client components (HomepageCtaSections, ServiceCtaWithModal)
 * are included for discoverability but should be imported directly in route files
 * to ensure proper tree-shaking.
 */
export { AreasWeServe } from "./AreasWeServe";
export { CredentialsSection } from "./CredentialsSection";
export { CtaSection } from "./CtaSection";
export type { CtaLink } from "./CtaSection";
export { FaqAccordion } from "./FaqAccordion";
export type { FaqItem } from "./FaqAccordion";
export { FaqPreview } from "./FaqPreview";
export { Hero } from "./Hero";
export { HomepageCtaSections } from "./HomepageCtaSections";
export { ProcessSteps } from "./ProcessSteps";
export { QuickResponseSection } from "./QuickResponseSection";
export { ReassuranceSection } from "./ReassuranceSection";
export { ReferralProfessionalsSection } from "./ReferralProfessionalsSection";
export { SectionDivider } from "./SectionDivider";
export type { SectionDividerVariant } from "./SectionDivider";
export { SectionHeader } from "./SectionHeader";
export { ServiceCard } from "./ServiceCard";
export { ServiceCards } from "./ServiceCards";
export { ServiceCtaWithModal } from "./ServiceCtaWithModal";
export { ServicePageLayout } from "./ServicePageLayout";
export type { ServicePageData, ServiceFeature } from "./ServicePageLayout";
export { StatsBar } from "./StatsBar";
export { Testimonials } from "./Testimonials";
export { TrustBadge } from "./TrustBadge";
export { TrustBar } from "./TrustBar";
export type { TrustItem } from "./TrustBar";
export { WhoWeHelp } from "./WhoWeHelp";
export { WhyDifferent } from "./WhyDifferent";
