...
const PERSONAL_SERVICE: ServiceDefinition = {
  ...
  faqs: [
    ...
    },
    {
      ...
      answer:
        "It depends on what you need and our current roster. We try to set up regular care quickly, and we aim to respond within 24 hours during business hours. We will always be honest if we can't start when you'd like — it's better than overpromising.",
    },
  ],
...
const HERO_TEMPLATES: Record<ServiceSlug, (region: string) => string> = {
  "ndis-services": (region) =>
    `Gentle Care Nursing is a registered NDIS provider supporting participants across ${region} with personalised in-home nursing, personal care and complex care. Our small, clinician-led team works closely with participants, families, plan managers and support coordinators to deliver consistent, person-centred support that fits each plan and each household. We deliberately keep our caseload small so participants get more dedicated care from familiar faces — not a rotating roster of agency staff. Across ${region}, families and coordinators choose Gentle Care for the same reasons: clear communication, reliable visits, and care that respects choice, culture and routine. Whether you're new to the NDIS, in the middle of a plan or coordinating support after a hospital stay, we're happy to have a calm, no-pressure conversation about how we might help.`,
...
function buildPage(
  ...
  const metaDescription =
    `${service.title} in ${region.region}, Sydney from Gentle Care Nursing. ` +
    `Clinician-led, relationship-based care with prompt response within business hours. ${region.positionLine}`;
  return {
